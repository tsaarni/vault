...
func New(p client.ConfigProvider, cfgs ...*aws.Config) (*IAM, error) {
	c := p.ClientConfig(EndpointsID, cfgs...)
	return newClient(*c.Config, c.Handlers, c.Endpoint, c.SigningRegion, c.SigningName)
}

// newClient creates, initializes and returns a new service client instance.
func newClient(cfg aws.Config, handlers request.Handlers, endpoint, signingRegion, signingName string) (*IAM, error) {
	svc := &IAM{
		Client: client.New(
			cfg,
			metadata.ClientInfo{
...
var BuildHandler = request.NamedHandler{Name: "awssdk.ram.Build", Fn: Build}
